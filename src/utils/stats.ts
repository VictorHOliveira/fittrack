...
export function calcularEvolucaoExercicio(
  ...
  for (const treino of historico) {
    const exercicio = treino.exercicios.find(e => e.exercicioId === exercicioId);
    if (!exercicio) continue;

    const cargaMax = Math.max(...exercicio.series.map(s => s.cargas));
    const volume = exercicio.series.reduce((acc, s) => acc + s.cargas * s.repeticoes, 0);
    const data = new Date(treino.dataExecucao).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
...
export function calcularFrequenciaSemanal(
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    labels.push(inicioSemana.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }));
...
export function calcularResumoPeriodo(
  ...
  const historicoFiltrado = historico.filter(t => new Date(t.dataExecucao) >= dataLimite);
  ...
  const totalVolume = historicoFiltrado.reduce(
    (acc, t) =>
      acc +
      t.exercicios.reduce(
        (a, e) => a + e.series.reduce((s, serie) => s + serie.cargas * serie.repeticoes, 0),
        0
      ),
    0
  );
  ...
  const cargas = historicoFiltrado.flatMap(t =>
    t.exercicios.flatMap(e => e.series.map(s => s.cargas)).filter(c => c > 0)
  );
...
export function listarExerciciosNoHistorico(historico: TreinoCompleto[]): string[] {
